...
import type {
  ...
  export async function deployArtifact(
    ...
    try {
      ...
      // Step 4: Create or get Vercel project
      emit('uploading', 'Creating Vercel project...', 30);
      const projectName = await client.generateUniqueProjectName(request.projectName);
      ...
      emit('uploading', `Created project: ${vercelProject.name}`, 40);
  
      // Step 5: Add environment variables if provided
      if (request.environmentVariables && Object.keys(request.environmentVariables).length > 0) {
        emit('uploading', 'Configuring environment variables...', 45);
        const envVars = Object.entries(request.environmentVariables).map(([key, value]) => ({
          key,
          value,
